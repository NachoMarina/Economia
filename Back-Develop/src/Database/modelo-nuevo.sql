...
FROM (
    ...
    /*Pasivos corrientes*/
     SELECT JSON_ARRAY(JSON_OBJECT('rubro', nombre_rubro, 'saldo', saldo)) AS pasivos_corrientes
FROM (
    SELECT R.nombre_rubro as nombre_rubro, SUM(C.saldo_cuenta) as saldo
    FROM tipo_cuentas TC
    INNER JOIN cuentas C ON C.id_cuenta = TC.id_cuenta
   ...
   SELECT JSON_ARRAY(JSON_OBJECT('rubro', nombre_rubro, 'saldo', saldo)) AS pasivos_no_corrientes
FROM (
    SELECT R.nombre_rubro as nombre_rubro, SUM(C.saldo_cuenta) as saldo
    FROM tipo_cuentas TC
    INNER JOIN cuentas C ON C.id_cuenta = TC.id_cuenta
...
SELECT
JSON_ARRAY(JSON_OBJECT('rubro', nombre_rubro, 'saldo', saldo)) AS resultado_del_ejercicio
FROM(
